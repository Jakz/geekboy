...
class Blip_Synth {
	static_assert( 1 <= quality && quality <= 5, "Quality must 1 <= x <= 5" );
	static_assert( -32768 <= range && range <= 32767, "Range must be -32768 < x < 32767" );
	enum {
		...
		int delta, Blip_Buffer* blip_buf ) const
{
	...
	unsigned sample_index = static_cast<unsigned>((time >> BLIP_BUFFER_ACCURACY) & ~1);
